...
public class AceServiceImpl implements AceService {
    ...
    private void removeAcesForAuthorizables(AcInstallationHistoryPojo history, Session session, Set<String> authorizablesInConfig,
        ...
        for (Map.Entry<String, Set<AceBean>> entry : repositoryDumpAceMap.entrySet()) {
            ...
            for (AceBean existingAce : existingAcl) {
                ...
                if (authorizablesInConfig.contains(existingAce.getPrincipalName()) && !honorService.checkHonorablesPathGroup(existingAce.getJcrPath(), existingAce.getPrincipalName())) {
    ...
    public void installConfigurationFiles(Session session, AcInstallationHistoryPojo history,
        ...
        try {
            ...
            if (configurationFileContentsByFilename != null) {
                ...
                setupHonorablesService(acConfiguration);
    ...
    private void setupHonorablesService(AcConfiguration acConfiguration) {
        HashMap<String, Set<String>> honorsMap = new HashMap<>();
        Map<String, Set<AuthorizableConfigBean>> iterateOver = acConfiguration.getAuthorizablesConfig();

        for (Map.Entry<String, Set<AuthorizableConfigBean>> entry : iterateOver.entrySet()) {
            Set<String> honoredPaths = honorsMap.get(entry.getKey());
            if (honoredPaths == null) {
                honoredPaths = new HashSet<>();
            }
            for (AuthorizableConfigBean authorizable : iterateOver.get(entry.getKey())) {
                honoredPaths.addAll(Arrays.asList(authorizable.getHonorsStringFromConfig().replaceAll("\\s+", "").split(",")));
            }
            honoredPaths.remove("");
            if (!honoredPaths.isEmpty()) {
                honorsMap.put(entry.getKey(), honoredPaths);
            }
        }
        honorService.setupService(honorsMap);
    }
